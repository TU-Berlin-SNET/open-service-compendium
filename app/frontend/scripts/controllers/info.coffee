...
($scope, _) ->
        ...
        if ((oldValue != newValue) || (!_.isEmpty(oldValue)))
            ...
            for i, enumValue of $scope.enumerations
                ...
                if (enumValue.values)
                    ...
                    enumValue.statisticsInfo["Average Deviation from Uniform Distribution"] = deviation.toFixed(2)
    ...
    # Increment number of services in the corresponding category to the given property
    # Increment only if the correspoding category has not
    # been incremented for the property service yet (not checked)
    addPropertyToCategory = (property) ->
        for key, category of propertiesCategories
